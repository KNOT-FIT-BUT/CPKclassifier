        ...
        for pAndMeta in zip(predicted, *(metaForWrite[i] for i in metaForWrite.keys())):
            ...
            if targetsNames==[]:
                ...
                for b in range(1, showBest):
                    ...
                    if self.confPostfix:
                        dWrite[self.nBestPrefix+str(b+1)+self.confPostfix]=pAndMeta[0][targetsNames.index(nBest[b])]
            
                dWrite[self.predictedFieldName]=nBest[0]
                if self.confPostfix: 
                    dWrite[self.predictedFieldName+self.confPostfix]=pAndMeta[0][targetsNames.index(nBest[0])]
