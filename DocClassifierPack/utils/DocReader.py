...
class DocReader(object):
    ...
    def __init__(self, dataFile, metadataFile, lazyEvalData=False, nonEmpty=None, empty=None, fieldRegex=None, minPerField=None, maxPerField=None,
                  initMetadata=None, initLinesOffsets=None, itemDelimiter=None, selectWords=None, selectItems=None):
        """
    ...
    def __nonEmptyRegexRowFilter(self, row):
        """
    ...
    def __filter(self):
        ...
        with open(self.metadataFile, "r") as metadata:
            ...
            self.__validateMetaFields()
            #prvně čti a použij non empty a regex filter 
            lineCnt=0
            for row in reader:
                lineCnt=lineCnt+1
                    
                rowA=self.__nonEmptyRegexRowFilter(row)
                if rowA:
                    self.metadata.append((lineCnt-1, rowA))
    ...
    def __validateMetaFields(self):
        ...
        if self.nonEmpty:
            needFields.update(self.nonEmpty)
        
        if self.empty:
            needFields.update(self.empty)
...
class DocReaderDataString(list):
    """
